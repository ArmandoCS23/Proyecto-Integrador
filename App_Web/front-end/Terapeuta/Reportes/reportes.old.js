...
(function(){
  ...
  function readActivities(){ let out=[]; activitiesKeys.forEach(k=>{ const a=readJSON(k); if(Array.isArray(a)) out=out.concat(a); }); return out.filter(Boolean); }
    // read activities and restrict to assigned patients (if any)
  ...
  (function autoSelect(){ const params=new URLSearchParams(window.location.search); const pid = params.get('patientId'); if(pid){ renderPatientsList(); const found = (patients||[]).find(p=> String(p.id)===String(pid) || String(p.name)===String(pid)) || null; if(found) { setTimeout(()=> showReport(found), 50); } return; } renderPatientsList(); })();
